...
export default async function InvoicesPage({
  ...
  const rows = await q<{
    id: string; doc_no: string; partner_name: string; issue_date: string; due_date: string
    subtotal: string; vat_amount: string; total_amount: string; balance_due: string
    status: string; journal_entry_id: string | null; entry_no: string | null
  }>(`
    SELECT i.id::text, i.doc_no, p.partner_name, i.issue_date::text, i.due_date::text,
           i.subtotal::text, i.vat_amount::text, i.total_amount::text, i.balance_due::text,
           i.status::text, i.journal_entry_id::text, je.entry_no
      FROM acc.ar_invoices i
      JOIN acc.business_partners p ON p.id = i.customer_id
      LEFT JOIN acc.journal_entries je ON je.id = i.journal_entry_id
  ...
  return (
    <>
      <div className="page-head">
        <div>
          <h1>ใบแจ้งหนี้</h1>
          <p>ทั้งหมด {rows.length} ใบ · คงค้าง {baht(totalOpen)} บาท</p>
        </div>
        <div className="toolbar">
          <Link className="btn" href="/receipts/new">รับชำระเงิน</Link>
          <Link className="btn primary" href="/invoices/new">+ ออกใบแจ้งหนี้</Link>
        </div>
      </div>

      {sp.created && (
        <div className="alert ok">
          <div>ออกใบแจ้งหนี้ <strong>{sp.created}</strong> และลงบัญชีเรียบร้อยแล้ว</div>
        </div>
      )}
      ...
      <div className="card">
        <div className="table-wrap">
          {rows.length === 0 ? (
            <div className="empty">ยังไม่มีใบแจ้งหนี้</div>
          ) : (
                ...
                {rows.map((r) => {
                  ...
                  return (
                    <tr key={r.id}>
                      ...
                      <td data-label="คงค้าง" className="num">{Number(r.balance_due) > 0 ? baht(r.balance_due) : '—'}</td>
